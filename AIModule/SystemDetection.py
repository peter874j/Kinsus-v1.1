...
class Motion:
    ...
    def run(self, image):
        ...
        if self.processDetection.currentStep.number == 0 and self.waitStep1:
            if len(yoloResult["coordinate"]) == 1: 
                if not (Frame.BBoxInfo[1].contains_point(yoloResult["coordinate"][0].centerPoint) and yoloResult["label"][0] == "no_PCB"):
                    self.q = []
                    self.temp1 = False
                    self.haveAlarm = False
                    self.needRecord = False
    ...
    class __tool:
        ...
        def play_sound():
            pygame.mixer.init()
            pygame.mixer.music.load('./static/sound/motionAlert.mp3')
